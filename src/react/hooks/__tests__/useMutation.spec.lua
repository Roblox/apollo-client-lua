-- ROBLOX upstream: https://github.com/apollographql/apollo-client/blob/v3.4.0-rc.17/src/react/hooks/__tests__/useMutation.test.tsx
      ...
      createTodo(description: $description, priority: $priority) {
		...
		describe("General use", function()
				...
				return Promise.resolve()
					:andThen(function()
						...
						local function Component()
							...
							elseif renderCount == 1 then
								jestExpect(mutationFn).toBe(createTodo)
								jestExpect(loading).toBeTruthy()
								jestExpect(data).toBeUndefined()
							elseif renderCount == 2 then
								jestExpect(loading).toBeFalsy()
			...
			describe("mutate function upon error", function()
				itAsync(it)("resolves with the resulting data and errors", function(resolve, reject)
					return Promise.resolve():andThen(function()
						local variables = {
							description = "Get milk!",
						}
						...
						local function Component()
							local createTodo = useMutation(CREATE_TODO_MUTATION, {
								onError = function(_self, error_)
									jestExpect(error_.message).toEqual(CREATE_TODO_ERROR)
								end,
				...
				return Promise.new(function(resolve, reject)
					local function Component()
						...
						rejectOnComponentThrow(reject, function()
							...
							jestExpect(client).toBeDefined()
							jestExpect(client).toBeInstanceOf(ApolloClient)
						end)
						return nil
					end
      ...
      mutation DoSomething {
        doSomething {
          time
			...
			itAsync(it)("should be removed by default after the mutation", function(resolve, reject)
				...
				local function Component()
					...
					elseif condition == 2 then
						do
							...
							jestExpect(result.called).toBe(true)
							jestExpect(result.data).toBeUndefined()
						end
			...
			itAsync(it)("can be preserved by passing keepRootFields: true", function(resolve, reject)
				local timeReadCount = 0
				local timeMergeCount = 0
		...
		describe("Update function", function()
			itAsync(it)("should be called with the provided variables", function(resolve, reject)
				local variables = {
					description = "Get milk!",
				}
				...
				local function Component()
					local createTodo = useMutation(CREATE_TODO_MUTATION, {
						update = function(_self, _, __, options)
							jestExpect(options.variables).toEqual(variables)
				...
				return wait_(function()
					jestExpect(variablesMatched).toBe(true)
				end):andThen(resolve, reject)
			end)

			itAsync(it)("should be called with the provided context", function(resolve, reject)
				local context = { id = 3 }
				...
				local function Component()
					local createTodo = useMutation(CREATE_TODO_MUTATION, {
						context = context,
						update = function(_self, _, __, options)
							jestExpect(options.context).toEqual(context)
							foundContext = true
						end,
					})[1] :: MutationTupleFirst<any, any, any, any>
				...
				return wait_(function()
					jestExpect(foundContext).toBe(true)
				end):andThen(resolve, reject)
			end)

			describe("If context is not provided", function()
				itAsync(it)("should be undefined", function(resolve, reject)
					local variables = {
						description = "Get milk!",
					}
					...
					local function Component()
						local createTodo = useMutation(CREATE_TODO_MUTATION, {
							update = function(_self, _, __, options)
								jestExpect(options.context).toBeUndefined()
								checkedContext = true
							end,
						})[1] :: MutationTupleFirst<any, any, any, any>
					...
					return wait_(function()
						jestExpect(checkedContext).toBe(true)
					end):andThen(resolve, reject)
				end)
			end)
		end)
		...
		describe("Optimistic response", function()
			itAsync(it)("should support optimistic response handling", function(resolve, reject)
				...
				local function Component()
					...
					if condition == 0 then
						...
						jestExpect(dataInStore["Todo:1"]).toEqual(optimisticResponse.createTodo)
			...
			itAsync(it)("should be called with the provided context", function(resolve, reject)
				...
				return wait_(function()
					jestExpect(contextFn).toHaveBeenCalledTimes(2)
					jestExpect(contextFn).toHaveBeenCalledWith(context)
				end):andThen(resolve, reject)
			end)
		end)
        ...
        query Count { todoCount @client }
				...
				local function Component()
					...
					local condition = renderCount
					if condition == 1 then
						jestExpect(count.loading).toBe(false)
						jestExpect(count.data).toEqual({ todoCount = 0 })
					...
					elseif condition == 2 then
						...
						jestExpect(finishedReobserving).toBe(false)
					elseif condition == 3 then
						jestExpect(count.loading).toBe(false)
						jestExpect(count.data).toEqual({ todoCount = 1 })
					...
					elseif condition == 4 then
						...
						jestExpect(finishedReobserving).toBe(true)
					else
						reject("too many renders")
					end
				...
				return wait_(function()
					return onUpdatePromise:andThen(function(results)
						...
						jestExpect(results.diff).toEqual({
							complete = true,
							result = {
								todoCount = 1,
							},
						...
						jestExpect(results.result).toEqual({
							loading = false,
      ...
      query getTodos {
        todos {
          ...
          priority
			...
			itAsync(it)("refetchQueries with operation names should update cache", function(resolve, reject)
				...
				local function QueryComponent()
					...
					elseif condition == 2 then
						jestExpect(loading).toBe(false)
						jestExpect(data).toEqual(mocks[1].result.data)
						setTimeout(function()
							act(function()
					...
					elseif condition == 5 then
						jestExpect(loading).toBe(false)
						jestExpect(data).toEqual(mocks[3].result.data)
					else
						reject("too many renders")
					end
			...
			itAsync(it)("refetchQueries with document nodes should update cache", function(resolve, reject)
				local variables = { description = "Get milk!" }
				local mocks: Array<any> = {
					{
						request = {
			...
			itAsync(itFIXME)("refetchQueries should update cache after unmount", function(resolve, reject)
				local variables = { description = "Get milk!" }
				local mocks: Array<any> = {
					{
						request = {
				...
				unmount = jest.fn(
					render(
						React.createElement(ApolloProvider, { client = client }, React.createElement(QueryComponent))
					).unmount
				)
