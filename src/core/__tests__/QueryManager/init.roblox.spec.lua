...
local itAsync = require(srcWorkspace.utilities.testing.itAsync).itAsync
...
describe("QueryManager", function()
	...
	itAsync("handles GraphQL errors", function(resolve, reject)
		assertWithObserver({
			reject = reject,
			query = gql([[
	...
	itAsync("handles GraphQL errors as data", function(resolve, reject)
		assertWithObserver({
			reject = reject,
			query = gql([[
          ...
          allPeople(first: 1) {
			...
			observer = {
				next = function(_self, ref)
					...
					expect((errors :: FIX_ANALYZE)[1].message).toBe("This is an error message.")
					resolve()
				end,
				error = function(_self, apolloError)
	...
	itAsync("handles GraphQL errors with data returned", function(resolve, reject)
		assertWithObserver({
			reject = reject,
			query = gql([[
	...
	itAsync("empty error array (handle non-spec-compliant server) #156", function(resolve, reject)
		assertWithObserver({
			reject = reject,
			query = gql([[
	...
	itAsync("error array with nulls (handle non-spec-compliant server) #1185", function(resolve, reject)
		assertWithObserver({
			reject = reject,
			query = gql([[
          ...
          allPeople(first: 1) {
			...
			observer = {
				...
				error = function(_self, error_)
					expect((error_ :: any).graphQLErrors).toEqual({ NULL })
					expect(error_.message).toBe("Error message not found.")
					resolve()
				end,
			},
		})
	...
	itAsync("handles network errors", function(resolve, reject)
		assertWithObserver({
			reject = reject,
			query = gql([[
          ...
          allPeople(first: 1) {
			...
			observer = {
				...
				error = function(_self, error_)
					...
					expect((apolloError.networkError :: any).message).toMatch("Network error")
					resolve()
				end,
			},
		})
	...
	itAsync("uses console.error to log unhandled errors", function(resolve, reject)
		...
		setTimeout(
			function()
				expect(printed[1]).toMatch(RegExp("error"))
	...
	itAsync.skip("handles an unsubscribe action that happens before data returns", function(resolve, reject)
	...
	itAsync("causes link unsubscription if unsubscribed", function(resolve, reject)
		local expResult = {
			data = {
				allPeople = {
					people = { {
	...
	-- ROBLOX deviation START: RXJS tests not required
	-- itAsync.skip(
	...
	itAsync("allows you to subscribe twice to one query", function(resolve, reject)
		local request = {
			query = gql([[
	...
	itAsync("resolves all queries when one finishes after another", function(resolve, reject)
		local request = {
			query = gql([[
	...
	itAsync("allows you to refetch queries", function(resolve, reject)
		local request = {
			query = gql([[
		...
		return observableToPromise({ observable = observable }, function(result)
			...
			return expect(stripSymbols(result.data)).toEqual(data2)
		end):andThen(resolve, reject)
	end)

	itAsync("will return referentially equivalent data if nothing changed in a refetch", function(resolve, reject)
		local request = {
			query = gql([[
			...
			next = function(_self, result)
				xpcall(function()
					...
					elseif condition == 1 then
						expect(stripSymbols(result.data)).toEqual(data2)
						expect(result.data).never.toEqual(firstResultData)
						expect(result.data.b).toEqual(firstResultData.b)
						expect(result.data.d).never.toEqual(firstResultData.d)
          ...
          d {
			...
			observable:subscribe({
				next = function(_self, result)
					xpcall(function()
						expect(stripSymbols(result.data)).toEqual(data1)
						expect(stripSymbols(result.data)).toEqual(stripSymbols(observable:getCurrentResult().data))
						resolve()
					end, function(error_)
						reject(error_)
					end)
	...
	itAsync("sets networkStatus to `refetch` when refetching", function(resolve, reject)
		...
		return observableToPromise({ observable = observable }, function(result)
			...
			return expect(result.networkStatus).toBe(NetworkStatus.refetch)
		end, function(result)
	...
	itAsync("allows you to refetch queries with promises", function(resolve, reject)
		local request = {
			query = gql([[
	...
	itAsync("allows you to refetch queries with new variables", function(resolve, reject)
		local query = gql([[
	...
	itAsync("only modifies varaibles when refetching", function(resolve, reject)
		local query = gql([[
	...
	itAsync("continues to poll after refetch", function(resolve, reject)
		local query = gql([[
	...
	itAsync("sets networkStatus to `poll` if a polling query is in flight", function(resolve, reject)
		local query = gql([[
	...
	itAsync("can handle null values in arrays (#1551)", function(resolve, reject)
		local query = gql([[
	...
	itAsync("supports cache-only fetchPolicy fetching only cached data", function(resolve, reject)
		...
		return queryManager
			...
			:andThen(function()
				...
				return handle:result():andThen(function(result)
					...
					expect(result.data).never.toHaveProperty("vader")
				end)
			end)
			:andThen(resolve, reject)
	end)

	itAsync("runs a mutation", function(resolve, reject)
	...
	itAsync("runs a mutation even when errors is empty array #2912", function(resolve, reject)
		return assertMutationRoundtrip(resolve, {
			reject = reject,
			mutation = gql([[
        mutation makeListPrivate {
	...
	itAsync('runs a mutation with default errorPolicy equal to "none"', function(resolve, reject)
        ...
        mutation makeListPrivate {
          makeListPrivate(id: "5")
			...
			:andThen(function(result)
				...
				expect(error_.graphQLErrors).toEqual(errors)
			end)
			:andThen(resolve, reject)
	end)

	itAsync("runs a mutation with variables", function(resolve, reject)
		return assertMutationRoundtrip(resolve, {
			reject = reject,
			mutation = gql([[
	...
	itAsync("runs a mutation with object parameters and puts the result in the store", function(resolve, reject)
		local data = {
			makeListPrivate = {
				id = "5",
				isPrivate = true,
	...
	itAsync("runs a mutation and puts the result in the store", function(resolve, reject)
		local data = {
			makeListPrivate = {
				id = "5",
				isPrivate = true,
	...
	itAsync("runs a mutation and puts the result in the store with root key", function(resolve, reject)
		local mutation = gql([[
	...
	itAsync("doesn't return data while query is loading", function(resolve, reject)
		local query1 = gql([[
	...
	itAsync("updates result of previous query if the result of a new query overlaps", function(resolve, reject)
		local query1 = gql([[
	...
	itAsync("warns if you forget the template literal tag", function(resolve, reject)
		local queryManager = mockQueryManager(reject)
		expect(function()
			queryManager:query({
		...
		-- ROBLOX deviation START: separate into multiple steps as jest-roblox doesn't support expect(...).rejects functionality (https://jestjs.io/docs/tutorial-async#rejects)
	...
	itAsync("should transform queries correctly when given a QueryTransformer", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should transform mutations correctly", function(resolve, reject)
		local mutation = gql([[
	...
	itAsync("should reject a query promise given a network error", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should reject a query promise given a GraphQL error", function(resolve, reject)
		local query = gql([[
		...
		return mockQueryManager(reject, {
			...
			:andThen(
				...
				function(error_)
					...
					expect(not Boolean.toJSBoolean(apolloError.networkError)).toBeTruthy()
				end
			)
			:andThen(resolve, reject)
	end)
				...
				:andThen(function(result)
					...
					queryManager
						...
						:catch(function()
							...
							expect((queryManager.cache:extract().ROOT_QUERY :: any).author).toEqual(data.author)
							resolve()
						end)
				end)
				:catch(function()
	...
	itAsync("should be able to unsubscribe from a polling query subscription", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should not empty the store when a polling query fails due to a network error", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should not fire next on an observer if there is no change in the result", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should not write unchanged network results to cache", function(resolve, reject)
		...
		subscribeAndCount(reject, obsA, function(count, result)
			...
			elseif count == 4 then
				...
				reject(Error.new(("Unexpected %s"):format(HttpService:JSONEncode({ count = count, result = result }))))
			end
		end)
	...
	itAsync("should disable feud-stopping logic after evict or modify", function(resolve, reject)
		local cache = InMemoryCache.new({
			typePolicies = {
				Query = {
					fields = {
	...
	itAsync("should not error when replacing unidentified data with a normalized ID", function(resolve, reject)
      ...
      query {
							...
							author = {
								merge = function(_self, existing, incoming, ref)
									...
									elseif condition == 2 then
										...
										expect(ref:readField("id", incoming)).toBe("129")
										expect(ref:readField("name", incoming)).toEqual(dataWithId.author.name)
									else
										fail("unreached")
									end
									return incoming
		...
		return Promise.all({
			observableToPromise({ observable = observableWithoutId :: FIX_ANALYZE }, function(result)
				return expect(stripSymbols(result.data)).toEqual(dataWithoutId)
			end),
			observableToPromise({ observable = observableWithId :: FIX_ANALYZE }, function(result)
				return expect(stripSymbols(result.data)).toEqual(dataWithId)
			end),
		}):andThen(resolve, reject)
	end)

	itAsync("exposes errors on a refetch as a rejection", function(resolve, reject)
		local request = {
			query = gql([[
		...
		local function checkError(error_)
			expect(error_.graphQLErrors[1].message).toEqual("This is not the person you are looking for.")
		end
	...
	itAsync("does not return incomplete data when two queries for the same item are executed", function(resolve, reject)
		local queryA = gql([[
      ...
      query queryB {
		...
		return Promise.all({
			observableToPromise({ observable = observableA :: FIX_ANALYZE }, function()
				expect(stripSymbols(getCurrentQueryResult(observableA))).toEqual({
					data = dataA,
					partial = false,
				})
	...
	itAsync('only increments "queryInfo.lastRequestId" when fetching data from network', function(resolve, reject)
		local query = gql([[
	...
	describe("polling queries", function()
		itAsync("allows you to poll queries", function(resolve, reject)
			local query = gql([[
        ...
        query fetchLuke($id: String) {
          people_one(id: $id) {
			...
			return observableToPromise({ observable = observable }, function(result)
				return expect(stripSymbols(result.data)).toEqual(data1)
			end, function(result)
				return expect(stripSymbols(result.data)).toEqual(data2)
			end):andThen(resolve, reject)
		end)

		itAsync("does not poll during SSR", function(resolve, reject)
			local query = gql([[
		...
		itAsync("allows you to unsubscribe from polled queries", function(resolve, reject)
			local query = gql([[
		...
		itAsync("allows you to unsubscribe from polled query errors", function(resolve, reject)
			local query = gql([[
		...
		itAsync("exposes a way to start a polling query", function(resolve, reject)
			local query = gql([[
		...
		itAsync("exposes a way to stop a polling query", function(resolve, reject)
			local query = gql([[
		...
		itAsync("stopped polling queries still get updates", function(resolve, reject)
			local query = gql([[
	...
	describe("store resets", function()
		itAsync("returns a promise resolving when all queries have been refetched", function(resolve, reject)
			local query = gql([[
          ...
          author2 {
			...
			return Promise.all({
				observableToPromise({ observable = observable :: FIX_ANALYZE }, function(result)
					return expect(stripSymbols(result.data)).toEqual(data)
				end),
		...
		itAsync("should change the store state to an empty state", function(resolve, reject)
			local queryManager = createQueryManager({
        ...
        query {
          author {
			...
			end) :: any --[[ ROBLOX TODO: timeout is valid method in Lua Promise, fix in Polyfill type ]]):timeout(3)
		end)

		itAsync("should not refetch torn-down queries", function(resolve, reject)
			local queryManager: QueryManager<NormalizedCacheObject>
		...
		itAsync("should not error when resetStore called", function(resolve, reject)
			local query = gql([[
		...
		itAsync("should not error on a stopped query()", function(resolve, reject)
			local queryManager: QueryManager<NormalizedCacheObject>
			local query = gql([[
		...
		itAsync("should throw an error on an inflight fetch query if the store is reset", function(resolve, reject)
			local query = gql([[
        ...
        query {
          author {
			...
			queryManager
				...
				:catch(function(error_)
					expect(error_.message).toMatch("Store reset")
					resolve()
				end)
		...
		itAsync("should call refetch on a mocked Observable if the store is reset", function(resolve, reject)
			local query = gql([[
		...
		itAsync("should not call refetch on a cache-only Observable if the store is reset", function(resolve, reject)
			local query = gql([[
		...
		itAsync("should not call refetch on a standby Observable if the store is reset", function(resolve, reject)
			local query = gql([[
		...
		itAsync("should throw an error on an inflight query() if the store is reset", function(resolve, reject)
			local queryManager: QueryManager<NormalizedCacheObject>
			local query = gql([[
	...
	describe("refetching observed queries", function()
		itAsync("returns a promise resolving when all queries have been refetched_", function(resolve, reject)
			local query = gql([[
		...
		itAsync("should only refetch once when we refetch observable queries", function(resolve, reject)
			local queryManager: QueryManager<NormalizedCacheObject>
			local query = gql([[
		...
		itAsync("should not refetch torn-down queries_", function(resolve, reject)
			local queryManager: QueryManager<NormalizedCacheObject>
			local observable: ObservableQuery_<any>
			local query = gql([[
		...
		itAsync("should not error after reFetchObservableQueries", function(resolve, reject)
			local query = gql([[
		...
		itAsync("should not call refetch on a non-subscribed Observable", function(resolve, reject)
			local query = gql([[
	...
	describe("refetching specified queries", function()
		itAsync("returns a promise resolving when all queries have been refetched__", function(resolve, reject)
			local query = gql([[
          ...
          author2 {
			...
			return Promise.all({
				...
				:andThen(function()
					...
					queryManager:refetchQueries({ include = { "GetAuthor", "GetAuthor2" } }):forEach(function(result)
	...
	describe("loading state", function()
		itAsync("should be passed as false if we are not watching a query", function(resolve, reject)
			local query = gql([[
        ...
        query {
          fortuneCookie
			...
			return mockQueryManager(reject, {
				...
				:query({ query = query })
				:andThen(function(result)
					expect(not result.loading).toBeTruthy()
					expect(stripSymbols(result.data)).toEqual(data)
				end)
				:andThen(resolve, reject)
		end)

		itAsync("should be passed to the observer as true if we are returning partial data", function(resolve, reject)
		...
		itAsync("should be passed to the observer as false if we are returning all the data", function(resolve, reject)
			assertWithObserver({
				reject = reject,
				query = gql([[
          ...
          query {
            author {
              ...
              lastName
				...
				observer = {
					next = function(_self, result)
						expect(not Boolean.toJSBoolean(result.loading)).toBeTruthy()
						resolve()
					end,
				},
			})
		...
		itAsync("will update on `resetStore`", function(resolve, reject)
			local testQuery = gql([[
		...
		itAsync("will be true when partial data may be returned", function(resolve, reject)
			local query1 = gql([[{
        a { x1 y1 z1 }
      }]])
			local query2 = gql([[{
	...
	describe("refetchQueries", function()
		...
		itAsync("should refetch the right query when a result is successfully returned", function(resolve, reject)
			local mutation = gql([[
        ...
        query getAuthors($id: ID!) {
          author(id: $id) {
			...
			return observableToPromise({
				...
				expect(stripSymbols(observable:getCurrentResult().data)).toEqual(secondReqData)
				expect(stripSymbols(result.data)).toEqual(secondReqData)
			end):andThen(resolve, reject)
		end)

		itAsync("should not warn and continue when an unknown query name is asked to refetch", function(resolve, reject)
			local mutation = gql([[
		...
		itAsync("also works with a query document and variables", function(resolve, reject)
			local mutation = gql([[
					...
					return Promise.new(function(res)
						...
						:andThen(function()
							...
							queryManager["queries"]:forEach(function(queryInfo, queryId)
								expect(queryId).never.toContain("legacyOneTimeQuery")
							end)
						end)
						:andThen(resolve, reject)
				else
		...
		itAsync("also works with a conditional function that returns false", function(resolve, reject)
			local mutation = gql([[
		...
		itAsync("also works with a conditional function that returns an array of refetches", function(resolve, reject)
			local mutation = gql([[
        ...
        query getAuthors {
          author {
			...
			return observableToPromise({ observable = observable }, function(result)
				...
				return expect(stripSymbols(result.data)).toEqual(secondReqData)
			end):andThen(resolve, reject)
		end)

		itAsync("should refetch using the original query context (if any)", function(resolve, reject)
			local mutation = gql([[
        ...
        query getAuthors($id: ID!) {
          author(id: $id) {
			...
			return observableToPromise({ observable = observable }, function(result)
				...
				expect(context.headers).never.toBeUndefined()
				expect(context.headers.someHeader).toEqual(headers.someHeader)
			end):andThen(resolve, reject)
		end)

		itAsync("should refetch using the specified context, if provided", function(resolve, reject)
			local mutation = gql([[
	...
	describe("onQueryUpdated", function()
      ...
      query getAuthors($id: ID!) {
		...
		itAsync.skip("should refetch using the specified context, if provided", function(resolve, reject)
			local queryManager = makeQueryManager(reject)
	...
	describe("awaitRefetchQueries", function()
          ...
          changeAuthorName(newName: "Jack Smith") {
            ...
            lastName
				...
				return observableToPromise({ observable = observable }, function(result)
						...
						if awaitRefetchQueries then
							expect(mutationComplete).never.toBeTruthy()
						else
		...
		it("should allow catching errors from `refetchQueries` when " .. "`awaitRefetchQueries` is `true`", function()
			return awaitRefetchTest({ awaitRefetchQueries = true, testQueryError = true }):timeout(3):expect()
		end)
	end)

	describe("store watchers", function()
		itAsync("does not fill up the store on resolved queries", function(resolve, reject)
			local query1 = gql([[
          ...
          four
			...
			return queryManager
				...
				:andThen(function()
					...
					expect((cache :: any).watches.size).toBe(0)
				end)
				:andThen(resolve, reject)
		end)
	end)
	...
	describe("client awareness", function()
		itAsync("should pass client awareness settings into the link chain via context", function(resolve, reject)
			local query = gql([[
