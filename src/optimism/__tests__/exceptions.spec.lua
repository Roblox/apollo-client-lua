...
return function()
	...
	describe("exceptions", function()
		it("should be cached", function()
			...
			wrapper:dirty()
			jestExpect(wrapper()).toBe("already threw")
			jestExpect(wrapper()).toBe("already threw")
			wrapper:dirty()
			jestExpect(wrapper()).toBe("already threw")
		end)
		...
		it("should memoize a throwing fibonacci function", function()
			...
			local function check(n: number, expected: number)
				...
				if not ok then
					-- ROBLOX deviation: we can't throw bare-bone number. We need to at least wrap them in an Array
					jestExpect(result[1]).toBe(expected)
				end
			end

			-- ROBLOX deviation: we can't calculate more than fib(58) because of Stack Oveflow 🤯
