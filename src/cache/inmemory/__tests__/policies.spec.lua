...
describe("type policies", function()
			...
			typePolicies = {
				Book = {
					keyFields = function(self, book, context)
						...
						expect((context.selectionSet :: any).kind).toBe("SelectionSet")
	...
	describe("field policies", function()
		...
		it("can include optional arguments in keyArgs", function()
			...
			expect(Array.sort(storeFieldNames)).toEqual({
				"writings",
						...
						fields = {
							comments = {
								keyArgs = function(args, context)
									...
									expect((context.field :: any).name.value).toBe("comments")
		...
		it("read, merge, and modify functions can access options.storage", function()
			...
			expect(cache:readQuery({ query = gql("query { mergeRead mergeReadModify }") })).toEqual({
				mergeRead = 1,
				mergeReadModify = 100,
			})
		...
		withErrorSpy(it, "readField helper function calls custom read functions", function()
			...
			expect(secondResult).never.toBe(firstResult)
			...
			expect((secondResult :: any).agenda.tasks[1]).never.toBe((firstResult :: any).agenda.tasks[1])

			expect((secondResult :: any).agenda.tasks[2]).toBe((firstResult :: any).agenda.tasks[2])

			expect((secondResult :: any).agenda.tasks[3]).never.toBe((firstResult :: any).agenda.tasks[3])

			expect((secondResult :: any).agenda.tasks[4]).toBe((firstResult :: any).agenda.tasks[4]);

			(ownTimes["grandchild task"] :: any)(7)

			local thirdResult = read()

			expect(thirdResult).never.toBe(secondResult)
			...
			expect(fourthResult).never.toBe(thirdResult)
			...
			expect(fifthResult).never.toBe(fourthResult)
							...
							todos = {
								...
								read = function(_self, existing: Array<any>, ref_)
									...
									Array.forEach(slice, function(ref)
										return expect(ref_:isReference(ref)).toBe(true)
									end)
									return slice
								end,
								merge = function(_self, existing: Array<any>, incoming: Array<any>, ref)
									...
									Array.forEach(copy, function(todo)
										...
										return expect(todo == nil or ref:isReference(todo)).toBe(true)
									end)
									return copy
								end,
							},
		...
		itAsync("can handle Relay-style pagination", function(resolve, reject)
					...
					Query = {
						fields = {
							search = relayStylePagination(function(args, ref)
								...
								expect(typeof((args :: any).query)).toBe("string")
			...
			subscribeAndCount(reject, observable, function(count, result)
				...
				elseif count == 4 then
					...
					expect(result.data.search.edges).toEqual(Array.concat({}, firstEdges, secondEdges))
					expect(cache:extract()).toMatchSnapshot()
				...
				elseif count == 5 then
					...
					client
						...
						:andThen(function(result)
							...
							expect((snapshot.ROOT_QUERY :: any)["search:james turrell"]).toEqual({
						...
						fields = {
							author = {
								merge = function(_self, existing: StoreObject, incoming: StoreObject, ref)
									expect(ref:mergeObjects(nil :: any, nil)).toBe(nil)
									expect(function()
										ref:mergeObjects(
									...
									expect(ref:mergeObjects(a, b)).toBe(b)
									expect(ref:mergeObjects(b, a)).toBe(a)
	...
	it("can read from foreign references using read helper", function()
		...
		expect(secondFirstBookResult).never.toBe(firstBookResult)
	...
	withWarningSpy(it, "readField warns if explicitly passed undefined `from` option", function()
		...
		expect(cache:readQuery({ query = fullNamesQuery })).toEqual({
