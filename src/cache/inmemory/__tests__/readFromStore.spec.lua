...
return function()
	...
	-- ROBLOX TODO: use real type when implemented
	-- type StoreObject = require(script.Parent.Parent.types).StoreObject
	type StoreObject = any
	...
	-- ROBLOX TODO: import when available
	-- local helpersModule = require(script.Parent.helpers)
	local helpersModule = {
		defaultNormalizedCacheFactory = function(...: any)
			error("not implemented")
			return {}
		end,
		readQueryFromStore = function(...: any)
			error("not implemented")
			return {}
		end,
		withError = function(...: any)
			error("not implemented")
			return {}
		end,
	}
	...
	local withErrorSpy = function(...: any)
		return {}
	end
	...
	xdescribe("reading from the store", function()
		...
		it("runs a nested query with proper fragment fields in arrays", function()
			withError(function()
				local store = defaultNormalizedCacheFactory({
					ROOT_QUERY = { __typename = "Query", nestedObj = makeReference("abcde") } :: StoreObject,
					abcde = { id = "abcde", innerArray = { { id = "abcdef", someField = 3 } } } :: StoreObject,
				})
		...
		it("runs a basic query", function()
			local result =
				{ id = "abcd", stringField = "This is a string!", numberField = 5, nullField = nil } :: StoreObject
      ...
      query someBigQuery(
		...
		it("runs a nested query", function()
			local result: any = {
				...
				numberField = 5,
				nullField = nil,
				nestedObj = { id = "abcde", stringField = "This is a string too!", numberField = 6, nullField = nil } :: StoreObject,
			}
			...
			local store = defaultNormalizedCacheFactory({
				ROOT_QUERY = assign(
					...
					{ nestedObj = makeReference("abcde") } :: StoreObject
				),
		...
		it("runs a nested query with multiple fragments", function()
			local result: any = {
				...
				nullField = nil,
				nestedObj = { id = "abcde", stringField = "This is a string too!", numberField = 6, nullField = nil } :: StoreObject,
				deepNestedObj = { stringField = "This is a deep string", numberField = 7, nullField = nil } :: StoreObject,
				nullObject = nil,
				__typename = "Item",
			}
			...
			local store = defaultNormalizedCacheFactory({
				ROOT_QUERY = assign(
					...
					{ __typename = "Query", nestedObj = makeReference("abcde") } :: StoreObject
				),
          ...
          ... on Query {
			...
			jestExpect(stripSymbols(queryResult)).toEqual({
				...
				nestedObj = {
					...
					numberField = 6,
					nullField = nil,
					deepNestedObj = { stringField = "This is a deep string", numberField = 7, nullField = nil },
				},
				nullObject = nil,
			})
		end)
		...
		it("runs a nested query with an array without IDs", function()
			local result: any = {
				...
				nullField = nil,
				nestedArray = {
					{ stringField = "This is a string too!", numberField = 6, nullField = nil },
					{ stringField = "This is a string also!", numberField = 7, nullField = nil },
				} :: Array<StoreObject>,
			}
		...
		it("runs a nested query with an array without IDs and a null", function()
			local result: any = {
				...
				numberField = 5,
				nullField = nil,
				nestedArray = { nil, { stringField = "This is a string also!", numberField = 7, nullField = nil } } :: Array<StoreObject>,
			}
				...
				query = gql([[
        {
          ...
          nestedArray {
			...
			jestExpect(stripSymbols(queryResult)).toEqual({
				stringField = "This is a string!",
				numberField = 5,
				nestedArray = { nil :: any, { stringField = "This is a string also!", numberField = 7 } },
			})
		end)
		...
		it("runs a nested query with an array with IDs and a null", function()
			local result: any = {
				...
				nestedArray = {
					nil,
					{ id = "abcde", stringField = "This is a string also!", numberField = 7, nullField = nil },
				} :: Array<StoreObject>,
			}
			...
			local store = defaultNormalizedCacheFactory({
				ROOT_QUERY = assign(
					...
					{ nestedArray = { nil, makeReference("abcde") } }
				) :: StoreObject,
		...
		it("readQuery supports returnPartialData", function()
			...
			jestExpect(cache:readQuery({ query = bQuery })).toBe(nil)
			jestExpect(cache:readQuery({ query = abQuery })).toBe(nil)
		...
		it("readFragment supports returnPartialData", function()
			...
			jestExpect(cache:readFragment({ id = id, fragment = aFragment })).toBe(nil)
			jestExpect(cache:readFragment({ id = id, fragment = bFragment })).toBe(nil)
			jestExpect(cache:readFragment({ id = id, fragment = abFragment })).toBe(nil)
  ...
  "present": "also here"
		...
		it("runs a nested query where the reference is null", function()
			...
			local store = defaultNormalizedCacheFactory({
				ROOT_QUERY = assign({}, assign({}, omit(result, "nestedObj")), { nestedObj = nil }) :: StoreObject,
			})
		...
		it("read functions for root query fields work with empty cache", function()
			...
			})).toEqual({ uuid = "8d573b9c-cfcf-4e3e-98dd-14d255af577e", null = nil })
		...
		it("custom read functions can map/filter dangling references", function()
			local cache = InMemoryCache.new({
				typePolicies = {
					Query = {
						fields = {
							ducks = function(self, existing: Array<Reference>, ref)
								if existing == nil then
									existing = {}
								end
								local canRead = ref.canRead
								return Array.map(existing, function(duck)
									return (function()
										if Boolean.toJSBoolean(canRead(duck)) then
											return duck
										else
											return nil
										end
							...
							chickens = function(self, existing: Array<Reference>, ref)
								...
								return Array.map(existing, function(chicken)
									return (function()
										if Boolean.toJSBoolean(canRead(chicken)) then
											return chicken
										else
											return {}
										end
							...
							oxen = function(self, existing: Array<Reference>, ref)
								...
								local canRead = ref.canRead
								return Array.filter(existing, canRead)
							end,
						},
					},
				},
			...
			local cache = InMemoryCache.new({
				typePolicies = {
					Deity = {
						keyFields = { "name" },
						fields = {
							children = function(self, offspring: Array<Reference>, ref)
								local canRead = ref.canRead
								return (function()
									if Boolean.toJSBoolean(offspring) then
										return Array.filter(offspring, canRead)
									else
										return {}
									end
								end)()
					...
					Query = {
						fields = {
							ruler = function(self, ruler, ref)
								local canRead, toReference = ref.canRead, ref.toReference
								return (function()
									if Boolean.toJSBoolean(canRead(ruler)) then
										return ruler
									else
      ...
      query {
        ruler {
          ...
          children {
			...
			local childrenOfZeus = Array.map({ "Ares", "Artemis", "Apollo", "Athena" }, function(name)
				return { __typename = "Deity", name = name, children = {} }
			end)
		...
		it("returns === results for different queries", function()
			...
			jestExpect(bResult3).not_.toBe(bData3)
