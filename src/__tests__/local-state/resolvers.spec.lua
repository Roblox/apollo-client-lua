...
describe("Basic resolver capabilities", function()
	itAsync("should run resolvers for @client queries", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should handle queries with a mix of @client and server fields", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should have access to query variables when running @client resolvers", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should pass context to @client resolvers", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should handle resolvers that work with booleans properly", function(resolve, reject)
		local query = gql([[
      ...
      query CartDetails {
		...
		return client:query({ query = query, fetchPolicy = "network-only" }):andThen(function(ref)
			local data = ref.data
			expect(Object.assign({}, data)).toMatchObject({ isInCart = false })
			resolve()
		end)
	end)
	...
	it("should handle nested asynchronous @client resolvers (issue #4841)", function()
      ...
      query DeveloperTicketComments($id: ID) {
		...
		local function times<T>(n: number, thunk: () -> T): Promise<Array<T>>
			...
			local result: Array<Promise<T>> = {}
			for i = 1, n do
				local res = thunk()
				table.insert(
					result,
					if Promise.is(res) then (res :: any) :: Promise<T> else Promise.resolve(res) :: Promise<T>
				)
		...
		local function check(result: ApolloQueryResult<any>)
			return Promise.new(function(resolve)
				...
				expect(#result.data.developer.tickets).toBe(ticketsPerDev)
				local commentIds = Set.new()
				Array.forEach(result.data.developer.tickets, function(ticket: any)
					expect(ticket.__typename).toBe("Ticket")
					expect(#ticket.comments).toBe(commentsPerTicket)
      ...
      mutation start {
		...
		return client
			...
			:andThen(function(ref)
				local data = ref.data
				expect(Object.assign({}, data)).toMatchObject({ field = 1 })
			end)
			:expect()
	end)
...
describe("Resolving field aliases", function()
	itAsync("should run resolvers for missing client queries with aliased field", function(resolve, reject)
		-- expect.assertions(1);
		local query = gql([[
	...
	itAsync("should respect aliases for *nested fields* on the @client-tagged node", function(resolve, reject)
		local aliasedQuery = gql([[
		...
		local client = ApolloClient.new({
			...
			resolvers = {
				Query = {
					...
					fie = function()
						reject("Called the resolver using the alias' name, instead of " .. "the correct resolver name.")
					end,
				},
			},
		})
	...
	it("should pull initialized values for aliased fields tagged with @client " .. "from the cache", function()
		...
		return client
			...
			:andThen(function(ref)
				local data = ref.data
				expect(Object.assign({}, data)).toMatchObject({
...
describe("Async resolvers", function()
	itAsync("should support async @client resolvers", function(resolve, reject)
		local query = gql([[
	...
	itAsync("should support async @client resolvers mixed with remotely resolved data", function(resolve, reject)
		local query = gql([[
...
describe("LocalState helpers", function()
	describe("#shouldForceResolvers", function()
		it(
			"should return true if the document contains any @client directives " .. "with an `always` variable of true",
			function()
