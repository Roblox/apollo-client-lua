-- ROBLOX upstream: https://github.com/apollographql/apollo-client/blob/v3.4.0-rc.17/src/__tests__/optimistic.ts
...
return function()
	...
	local function assertions(count: number, fn: (...any) -> ...any)
		local originalJestExpect = jestExpect
		local spy = jest.fn(originalJestExpect)
		jestExpect = spy

		fn()

		jestExpect = originalJestExpect
		jestExpect(spy).toHaveBeenCalledTimes(count)
	end
    ...
    query todoList {
        ...
        __typename
			...
			local optimisticResponse2 = assign({}, optimisticResponse, {
				createTodo = assign(
					...
					{ id = "66", text = "Optimistically generated 2" }
				),
			})
			...
			describe("with `updateQueries`", function()
					...
					Promise.resolve():andThen(function()
						...
						assertions(6, function()
							...
							local ok, result = pcall(function()
								...
								jestExpect(#(dataInStore["TodoList5"] :: any).todos).toBe(4)
								jestExpect((dataInStore["Todo99"] :: any).text).toBe("Optimistically generated")
								promise:expect()
							end)
							if not ok then
								local err = result
								jestExpect(err).toBeInstanceOf(Error)
								jestExpect(err.message).toBe("forbidden (test error)")
								local dataInStore = (client.cache :: InMemoryCache):extract(true)
								jestExpect(#(dataInStore["TodoList5"] :: any).todos).toBe(3)
								jestExpect(stripSymbols(dataInStore)).never.toHaveProperty("Todo99")
							end
						end)
				...
				itAsync(it)("handles errors produced by one mutation in a series", function(resolve, reject)
					Promise.resolve():andThen(function()
						--[[
							ROBLOX deviation: using custom assertions function to verify nr of assertions called
							original code:
							jestExpect.assertions(10)
						]]
						assertions(10, function()
							...
							local dataInStore = (client.cache :: InMemoryCache):extract(true)
							jestExpect(#(dataInStore["TodoList5"] :: any).todos).toBe(5)
							jestExpect((dataInStore["Todo99"] :: any).text).toBe("Optimistically generated")
							jestExpect((dataInStore["Todo66"] :: any).text).toBe("Optimistically generated 2")
							...
							do
								...
								jestExpect(#(dataInStore_["TodoList5"] :: any).todos).toBe(4)
								jestExpect(stripSymbols(dataInStore_)).never.toHaveProperty("Todo99")
								jestExpect(dataInStore_).toHaveProperty("Todo66")
								jestExpect((dataInStore_["TodoList5"] :: any).todos).toContainEqual(
									makeReference("Todo66")
								)
								jestExpect((dataInStore_["TodoList5"] :: any).todos).never.toContainEqual(
						...
						Promise.resolve():andThen(function()
							...
							assertions(34, function()
								...
								local function checkBothMutationsAreApplied(expectedText1: any, expectedText2: any)
									local dataInStore = (client.cache :: InMemoryCache):extract(true)
									jestExpect(#(dataInStore["TodoList5"] :: any).todos).toBe(5)
									jestExpect(dataInStore).toHaveProperty("Todo99")
									jestExpect(dataInStore).toHaveProperty("Todo66")
									-- <any> can be removed once @types/chai adds deepInclude
									jestExpect((dataInStore["TodoList5"] :: any).todos).toContainEqual(
										makeReference("Todo66")
									)
									jestExpect((dataInStore["TodoList5"] :: any).todos).toContainEqual(
										makeReference("Todo99")
									)
									jestExpect((dataInStore["Todo99"] :: any).text).toBe(expectedText1)
									jestExpect((dataInStore["Todo66"] :: any).text).toBe(expectedText2)
								end
								local subscriptionHandle: Subscription
								...
								local promise = client
									...
									:andThen(function(res: any)
										...
										jestExpect(latestState[tostring(1)].loading).toBe(false)
										jestExpect(latestState[tostring(2)].loading).toBe(true)
										-- ROBLOX deviation END
								...
								local promise2 = client
									...
									:andThen(function(res: any)
										...
										-- ROBLOX deviation START: latestState is an object so we need to index like an object
										jestExpect(latestState[tostring(1)].loading).toBe(false)
										jestExpect(latestState[tostring(2)].loading).toBe(false)
										-- ROBLOX deviation END
								...
								jestExpect(mutationsState[tostring(1)].loading).toBe(true)
								jestExpect(mutationsState[tostring(2)].loading).toBe(true)
								-- ROBLOX deviation END
            ...
            fragment todoList on TodoList {
              todos {
				...
				itAsync(itSKIP)("handles a single error for a single mutation", function(resolve, reject)
					Promise.resolve():andThen(function()
						--[[
							ROBLOX deviation: using custom assertions function to verify nr of assertions called
							original code:
				...
				-- ROBLOX TODO: fragments are not supported yet
				itAsync(itSKIP)("handles errors produced by one mutation in a series", function(resolve, reject)
					Promise.resolve():andThen(function()
						--[[
							ROBLOX deviation: using custom assertions function to verify nr of assertions called
							original code:
							...
							assertions(34, function()
								...
								local promise = client
									...
									:andThen(function(res: any)
										...
										jestExpect(latestState[2].loading).toBe(false)
										jestExpect(latestState[3].loading).toBe(true)
								...
								local promise2 = client
									...
									:andThen(function(res: any)
										...
										local latestState = client.queryManager.mutationStore :: any
										jestExpect(latestState[2].loading).toBe(false)
										jestExpect(latestState[3].loading).toBe(false)
								...
								jestExpect(mutationsState[2].loading).toBe(true)
								jestExpect(mutationsState[3].loading).toBe(true)
							...
							return client:mutate({
								...
								update = function(_self, proxy: any, mResult: any)
									...
									if updateCount == 1 then
										...
										jestExpect(readText).toEqual(optimisticText)
									elseif updateCount == 2 then
										local incomingText = mResult.data.createTodo.todos[1].text
										jestExpect(readText).toEqual(incomingText)
									else
						...
						:andThen(function(client)
							...
							return client:mutate({
								...
								update = function(_self, proxy: any, mResult: any)
									...
									elseif updateCount == 2 then
										jestExpect(data.todos[1].text).toEqual(mResult.data.createTodo.todos[1].text)
										jestExpect(data.todos[1].text).toEqual(
      ...
      mutation createTodo($text: String) {
        ...
        __typename
			...
			-- ROBLOX TODO: fragments are not supported yet
			itAsync(itSKIP)("will use a passed variable in optimisticResponse", function(resolve, reject)
				Promise.resolve():andThen(function()
					--[[
						ROBLOX deviation: using custom assertions function to verify nr of assertions called
						original code:
					...
					assertions(6, function()
						...
						local promise = client:mutate({
							...
							optimisticResponse = optimisticResponse,
							update = function(_self, proxy: any, mResult: any)
								jestExpect(mResult.data.createTodo.id).toBe("99")
            ...
            fragment todoList on TodoList {
              todos {
                ...
                __typename
						...
						local dataInStore = (client.cache :: InMemoryCache):extract(true)
						jestExpect(#(dataInStore["TodoList5"] :: any).todos).toEqual(4)
						jestExpect((dataInStore["Todo99"] :: any).text).toEqual(
						...
						jestExpect(#newResult.data.todoList.todos).toEqual(4)

						-- Since we used `prepend` it should be at the front
						jestExpect(newResult.data.todoList.todos[1].text).toEqual(
							"This one was created with a mutation."
						)
					end)
      ...
      mutation createTodo {
        ...
        __typename
			...
			itAsync(itFIXME)("will insert a single itemAsync to the beginning", function(resolve, reject)
				Promise.resolve():andThen(function()
					--[[
						ROBLOX deviation: using custom assertions function to verify nr of assertions called
						original code:
						jestExpect.assertions(7)
					]]
					assertions(7, function()
						local subscriptionHandle: Subscription
						local client =
							setup(reject, { request = { query = mutation }, result = mutationResult }):expect()
						...
						local promise = client:mutate({
							...
							updateQueries = {
								todoList = function(_self, prev: any, options: any)
									...
									jestExpect(mResult.data.createTodo.id).toEqual("99")
						...
						local dataInStore = (client.cache :: InMemoryCache):extract(true)
						jestExpect(#(dataInStore["TodoList5"] :: any).todos).toEqual(4)
						jestExpect((dataInStore["Todo99"] :: any).text).toEqual("Optimistically generated")
			...
			itAsync(it)("two array insert like mutations", function(resolve, reject)
				Promise.resolve():andThen(function()
					--[[
						ROBLOX deviation: using custom assertions function to verify nr of assertions called
						original code:
						jestExpect.assertions(9)
					]]
					assertions(9, function()
						...
						local promise = client
							...
							:andThen(function(res: any)
								...
								jestExpect(#(currentDataInStore["TodoList5"] :: any).todos).toEqual(5)
								jestExpect((currentDataInStore["Todo99"] :: any).text).toEqual(
									"This one was created with a mutation."
								)
								jestExpect((currentDataInStore["Todo66"] :: any).text).toEqual(
									"Optimistically generated 2"
								)
								return res
							end)
						...
						local dataInStore = (client.cache :: InMemoryCache):extract(true)
						jestExpect(#(dataInStore["TodoList5"] :: any).todos).toEqual(5)
						jestExpect((dataInStore["Todo99"] :: any).text).toEqual("Optimistically generated")
						jestExpect((dataInStore["Todo66"] :: any).text).toEqual("Optimistically generated 2")
						...
						(subscriptionHandle :: any):unsubscribe()
						-- There should be one more todo item than before
						jestExpect(#newResult.data.todoList.todos).toEqual(5)

						-- Since we used `prepend` it should be at the front
						jestExpect(newResult.data.todoList.todos[1].text).toEqual("Second mutation.")
						jestExpect(newResult.data.todoList.todos[2].text).toEqual(
							"This one was created with a mutation."
						)
					end)
					resolve()
			...
			itAsync(it)("two mutations, one fails", function(resolve, reject)
				Promise.resolve():andThen(function()
					--[[
						ROBLOX deviation: using custom assertions function to verify nr of assertions called
						original code:
					...
					assertions(10, function()
						...
						local promise = client
							...
							:catch(function(err: any)
								-- it is ok to fail here
								jestExpect(err).toBeInstanceOf(Error)
								jestExpect(err.message).toEqual("forbidden (test error)")
								return nil
							end)
						...
						do
							local dataInStore_ = (client.cache :: InMemoryCache):extract(true)
							jestExpect(#(dataInStore_["TodoList5"] :: any).todos).toEqual(4)
							jestExpect(stripSymbols(dataInStore_)).never.toHaveProperty("Todo99")
							jestExpect(dataInStore_).toHaveProperty("Todo66")
							jestExpect((dataInStore_["TodoList5"] :: any).todos).toContainEqual(makeReference("Todo66"))
							jestExpect((dataInStore_["TodoList5"] :: any).todos).never.toContainEqual(
								makeReference("Todo99")
							)
						end
      ...
      mutation createTodo {
        ...
        __typename
			...
			itAsync(it)("will insert a single itemAsync to the beginning", function(resolve, reject)
				Promise.resolve():andThen(function()
					--[[
						ROBLOX deviation: using custom assertions function to verify nr of assertions called
						original code:
					...
					assertions(6, function()
						...
						local client = setup(
							reject,
							{ request = { query = mutation }, delay = 300, result = mutationResult }
						):expect()
						...
						local promise = client:mutate({
							...
							update = function(_self, proxy: any, mResult: any)
								...
								if Boolean.toJSBoolean(firstTime) then
									jestExpect(duration < 300).toBe(true)
									firstTime = false
								else
									jestExpect(duration > 300).toBe(true)
								end
						...
						client
							...
							:andThen(function(newResult: any)
								(subscriptionHandle :: any):unsubscribe()
								-- There should be one more todo item than before
								jestExpect(#newResult.data.todoList.todos).toBe(4)

								-- Since we used `prepend` it should be at the front
								jestExpect(newResult.data.todoList.todos[1].text).toBe(
									"This one was created with a mutation."
								)
							end)
							:expect()
			...
			-- ROBLOX TODO: fragments are not supported yet
			itAsync(itSKIP)("two array insert like mutations", function(resolve, reject)
				Promise.resolve():andThen(function()
					--[[
						ROBLOX deviation: using custom assertions function to verify nr of assertions called
						original code:
              ...
              todos {
                ...
                __typename
						...
						local promise = client
							...
							:andThen(function(res: any)
								local currentDataInStore = (client.cache :: InMemoryCache):extract(true)
								jestExpect(#(currentDataInStore["TodoList5"] :: any).todos).toBe(5)
								jestExpect((currentDataInStore["Todo99"] :: any).text).toBe(
									"This one was created with a mutation."
								)
								jestExpect((currentDataInStore["Todo66"] :: any).text).toBe(
									"Optimistically generated 2"
								)
								return res
							end)
						...
						(subscriptionHandle :: any):unsubscribe()
						-- There should be one more todo item than before
						jestExpect(#newResult.data.todoList.todos).toBe(5)

						-- Since we used `prepend` it should be at the front
						jestExpect(newResult.data.todoList.todos[1].text).toBe("Second mutation.")
						jestExpect(newResult.data.todoList.todos[2].text).toBe("This one was created with a mutation.")
					end)
			...
			-- ROBLOX TODO: fragments are not supported yet
			itAsync(itSKIP)("two mutations, one fails", function(resolve, reject)
				Promise.resolve():andThen(function()
					--[[
						ROBLOX deviation: using custom assertions function to verify nr of assertions called
						original code:
			...
			itAsync(it)("final update ignores optimistic data", function(resolve, reject)
				...
				jestExpect(optimisticDiffs).toEqual({})
				jestExpect(realisticDiffs).toEqual({})
				...
				jestExpect(realisticDiffs).toEqual({
					{ complete = false, missing = { jestExpect.anything() }, result = {} },
				})
				...
				return client
					:mutate({
						...
						update = wrapReject(function(_self, cache, mutationResult)
							updateCount += 1
							if updateCount == 1 then
								jestExpect(mutationResult).toEqual({ data = { addItem = optimisticItem } })
								...
								jestExpect(cache:extract(false)).toEqual(expected)
								jestExpect(cache:extract(true)).toEqual(expected)
							elseif updateCount == 2 then
								jestExpect(mutationResult).toEqual({ data = { addItem = mutationItem } })
					...
					:andThen(function(result)
						jestExpect(result).toEqual({ data = { addItem = mutationItem } })
					...
					:andThen(function()
						...
						jestExpect(realisticDiffs).toEqual({
							{ complete = false, missing = { jestExpect.anything() }, result = {} } :: FIX_ANALYZE,
							{ complete = true, result = { items = { mutationItem } } },
						})
					end)
					:andThen(resolve, reject)
    ...
    mutation submitComment($repoFullName: String!, $commentContent: String!) {
      submitComment(
		...
		itAsync(it)("can post a new comment", function(resolve, reject)
			Promise.resolve():andThen(function()
				--[[
					ROBLOX deviation: using custom assertions function to verify nr of assertions called
					original code:
				...
				assertions(1, function()
					...
					(subscriptionHandle :: any):unsubscribe()
					jestExpect(#newResult.data.entry.comments).toBe(2)
				end)
