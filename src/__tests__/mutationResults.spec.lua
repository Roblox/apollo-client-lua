-- ROBLOX upstream: https://github.com/apollographql/apollo-client/blob/v3.4.0-rc.17/src/__tests__/mutationResults.ts
...
return function()
    ...
    query todoList {
      ...
      noIdList: todoList(id: 6) {
		...
		itAsync(it)("correctly primes cache for tests", function(resolve, reject)
		...
		itAsync(it)("correctly integrates field changes by default", function(resolve, reject)
			local mutation = gql([[
        ...
        __typename
			...
			return obsQuery
				...
				:andThen(function(newResult: any)
					jestExpect(newResult.data.todoList.todos[1].completed).toBe(true)
				end)
				:andThen(resolve, reject)
		end)

		itAsync(it)("correctly integrates field changes by default with variables", function(resolve, reject)
			local query = gql([[
      ...
      mutation upload($signature: String!) {
			...
			obs:subscribe({
				next = function(_self, result)
					if count == 0 then
						...
						jestExpect((result.data :: any).mini.cover).toBe("image")
					...
					if count == 1 then
						jestExpect((result.data :: any).mini.cover).toBe("image2")
						resolve()
					end
					count += 1
				end,
		...
		itAsync(it)("should write results to cache according to errorPolicy", function(resolve, reject)
          ...
          name
				...
				client
					...
					:andThen(function()
						...
						jestExpect(error_.message).toBe(expectedFakeError.message)
					end)
					:expect()

				jestExpect(client.cache:extract()).toMatchSnapshot()
				...
				jestExpect(ignoreErrorsResult).toEqual({
					data = {
						newPerson = {
							__typename = "Person",
							name = "Jenn Creighton",
				...
				jestExpect(allErrorsResult).toEqual({
					data = {
						newPerson = {
							__typename = "Person",
							name = "Ellen Shapiro",
          ...
          __typename
				...
				return obsQuery
					...
					:catch(function(err)
						subscriptionHandle:unsubscribe()
						error(err)
					end) -- ROBLOX deviation END
					:andThen(function(result)
						jestExpect(result).toEqual(mutationTodoResult)
					end)
					:andThen(resolve, reject)
			end
		)
			...
			it("mutation update function receives result from cache", function()
				...
				return client
					...
					:andThen(function(ref)
						...
						-- write is finished.
						jestExpect(client.cache:extract()).toEqual({
							ROOT_MUTATION = {
								__typename = "Mutation",
							},
						})
						...
						return client
							:mutate({
								...
								update = function(_self, cache, ref)
									...
									jestExpect(mutationUpdateCount).toBe(1)
									jestExpect(__typename).toBe("MutationPayload")
									-- ROBLOX deviation: not Date object in Lua
									jestExpect(typeof(time)).never.toBe("DateTime")
									jestExpect(time).toBe(startTime)
									jestExpect(timeReadCount).toBe(0)
									jestExpect(timeMergeCount).toBe(0)
									jestExpect(cache:extract()).toEqual({})
								end,
							})
							:andThen(function(ref)
								...
								-- ROBLOX deviation: not Date object in Lua
								jestExpect(typeof(time)).never.toBe("DateTime")
								jestExpect(time).toBe(tonumber(startTime))
								jestExpect(timeReadCount).toBe(0)
								jestExpect(timeMergeCount).toBe(0)
								jestExpect(mutationUpdateCount).toBe(1)
								jestExpect(client.cache:extract()).toEqual({})
      ...
      mutation createTodo {
        ...
        __typename
			...
			itAsync(it)("analogous of ARRAY_INSERT", function(resolve, reject)
				local subscriptionHandle: Subscription
				local ref = setupObsQuery(reject, {
				...
				return obsQuery
						...
						return client:mutate({
							...
							updateQueries = {
								todoList = function(_self, prev, options)
									...
									jestExpect(mResult.data.createTodo.text).toBe(
					...
					:andThen(function(newResult: any)
						...
						jestExpect(newResult.data.todoList.todos[1].text).toBe("This one was created with a mutation.")
					end)
					:andThen(resolve, reject)
			end)

			itAsync(it)("does not fail if optional query variables are not supplied", function(resolve, reject)
				local subscriptionHandle: Subscription
        ...
        mutation createTodo($requiredVar: String!, $optionalVar: String) {
          ...
          __typename
			...
			itAsync(it)("does not fail if the query did not complete correctly", function(resolve, reject)
				local ref = setupObsQuery(reject, { request = { query = mutation }, result = mutationResult })
				local client, obsQuery = ref.client, ref.obsQuery
					...
					:mutate({
						...
						updateQueries = {
							todoList = function(_self, prev, options)
								local mResult = options.mutationResult :: any
								jestExpect(mResult.data.createTodo.id).toBe("99")
								jestExpect(mResult.data.createTodo.text).toBe("This one was created with a mutation.")
			...
			itAsync(it)("does not fail if the query did not finish loading", function(resolve, reject)
			...
			itAsync(it)("does not make next queries fail if a mutation fails", function(resolve, reject)
			...
			itAsync(it)("error handling in reducer functions", function(resolve, reject)
				local subscriptionHandle: Subscription
				local ref = setupObsQuery(reject, {
					request = { query = mutation },
					result = mutationResult,
				...
				return obsQuery
					...
					:andThen(function()
						...
						subscriptionHandle:unsubscribe()
						jestExpect(error_.message).toBe("Hello... It's me.")
					end)
					:andThen(resolve, reject)
			end)
		end)

		itAsync(it)("does not fail if one of the previous queries did not complete correctly", function(resolve, reject)
      ...
      mutation Reset {
        reset {
          echo
			...
			subscribeAndCount(reject, watchedQuery, function(count, result)
				if count == 1 then
					jestExpect(result.data).toEqual({ echo = "a" })
				elseif count == 2 then
					jestExpect(result.data).toEqual({ echo = "b" })
					client:mutate({
				...
				elseif count == 3 then
					jestExpect(result.data).toEqual({ echo = "0" })
					resolve()
				end
			end)
		...
		itAsync(it)("allows mutations with optional arguments", function(resolve, reject)
			local count = 0
				...
				link = ApolloLink.from({
					function(_self, ref)
						...
						return Observable.new(function(observer)
							...
							count += 1
							if condition == 0 then
								jestExpect(variables).toEqual({ a = 1, b = 2 })
								observer:next({ data = { result = "hello" } })
								observer:complete()
								return
							elseif condition == 1 then
								jestExpect(variables).toEqual({ a = 1, c = 3 })
								observer:next({ data = { result = "world" } })
								observer:complete()
								return
							elseif condition == 2 then
								jestExpect(variables).toEqual({ a = nil, b = 2, c = 3 })
								observer:next({ data = { result = "goodbye" } })
								observer:complete()
								return
							elseif condition == 3 then
								jestExpect(variables).toEqual({})
      ...
      mutation($a: Int!, $b: Int, $c: Int) {
		...
		itAsync(it)("allows mutations with default values", function(resolve, reject)
			local count = 0
      ...
      mutation($a: Int = 1, $b: String = "cheese", $c: Int) {
		...
		itAsync(it)("will pass null to the network interface when provided", function(resolve, reject)
			local count = 0
				...
				link = ApolloLink.from({
					function(_self, ref)
						...
						return Observable.new(function(observer)
							...
							count += 1
							if condition == 0 then
								jestExpect(variables).toEqual({ a = 1, b = 2, c = nil })
								observer:next({ data = { result = "hello" } })
								observer:complete()
								return
							end
							if condition == 1 then
								jestExpect(variables).toEqual({ a = 1, b = nil, c = 3 })
								observer:next({ data = { result = "world" } })
								observer:complete()
								return
							end
							if condition == 2 then
								jestExpect(variables).toEqual({ a = nil, b = nil, c = nil })
								observer:next({ data = { result = "moon" } })
								observer:complete()
								return
							else
